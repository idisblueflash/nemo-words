...
[[ADR-0002]] chose CMUdict (GA) + BEEP (RP) over Kaikki as the project's
...
[[investigation-001-lexical-sets-dictionary-comparison]].
[[0003-match-nucleus-against-arpabet-mrpa-tokens]] then decided the
...
- **[[US-017]]** and **[[US-018]]** are independent brand-parsing
...
- **[[US-020]]** and **[[US-021]]** are independent, file-I/O-free
  ...
  with US-017/US-018.
- **[[US-019]]** is the union point: it depends on US-017 and US-018 both
...
- **[[US-022]]** is the closing piece, not optional cleanup: US-019
...
- **[[US-023]]** is the display-side counterpart, optional but real: once
...
`en_US_RP_ipa.tsv`, referenced in [[FEAT-001]]'s "Why Kaikki" section).
[[0003-match-nucleus-against-arpabet-mrpa-tokens]] changed that to
...
- [ ] [[US-019]] Build `ga_rp.tsv` and rename the raw-token loader (depends on US-017 + US-018)
- [ ] [[US-020]] Bidirectional ARPABET ↔ IPA conversion
- [ ] [[US-021]] Bidirectional MRPA ↔ IPA conversion
- [ ] [[US-022]] Migrate `lookup-rows`/`word-matches?` to token-based matching (depends on US-019 + US-020 + US-021)
- [ ] [[US-023]] Display IPA in the `ipa-lookup` CLI subcommand (depends on US-019 + US-020 + US-021 + US-022)
...
- Re-run `clojure -M -m nemo-words.core populate-lexical-sets` against
  ...
  re-check whether [[US-015]]'s `lexical-sets-table` needs any
...
- A concretely-scoped story to translate the Lexical Sets Table's
  ...
  than converting per query at lookup time, as [[US-022]] does), if
