...
2. **Write the `Back` field** to the story text, verbatim (apply only a tiny grammar fix
   the user asked for — e.g. "need" → "needs"). The row is `Front\tBack`, tab-separated:
   ```
   <word>	<story sentence, plain text>
   ```
   No `<b>`/`<br>`, no `🎭` prefix, nothing but the sentence. If no row exists for the
   word, **append one**. If a row exists, patch it in place — don't duplicate it. If the
   old row still carries the legacy multi-segment HTML back (`say:`/`def:`/🔊/📖 or a
